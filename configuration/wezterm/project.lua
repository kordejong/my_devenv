...
-- The directory that contains all your Git "organizations".
local project_path = wezterm.home_dir .. "/development/project"

wezterm.log_info("project_path: " .. project_path)

-- TODO: Finetune for conventions, github vs gitlab, ...
local function repository_paths()
	-- Start with home dir
	local paths = { wezterm.home_dir }

	for _, organization_path in ipairs(wezterm.glob(project_path .. "/*")) do
		for _, owner_path in ipairs(wezterm.glob(organization_path .. "/*")) do
			for _, repository_path in ipairs(wezterm.glob(owner_path .. "/*")) do
				table.insert(paths, repository_path)
			end
		end
	end
...
function module.choose_project()
	local choices = {}
	for _, value in ipairs(repository_paths()) do
...
--   for _, dir in ipairs(wezterm.glob(project_path .. '/*')) do
