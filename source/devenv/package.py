...
def check_shared_libraries(
    ...
    shared_library_path_names, _ = \
        devenv.shared_libraries.shared_library_dependencies(target_path_names)
    _, external_shared_library_path_names, _ = \
        devenv.shared_libraries.split_shared_library_path_names(
    ...
    if not sys.platform == "win32":
        for shared_library_path_name in external_shared_library_path_names:
            if not devenv.filesystem.file_is_executable(
...
def check_executables(
        path_names):
    for path_name in path_names:
        if not devenv.filesystem.file_is_executable(path_name):
            raise RuntimeError(
                "Shared library {} is not executable".format(path_name))

    dynamically_linked_executable_path_names = [path_name for path_name in
        path_names if devenv.filesystem.file_is_dynamically_linked(path_name)]
...
def package_base_name(
    ...
    """
    project_file_contents_ = devenv.project.project_file_contents(project_name)
    base_name = cmake_variable_value(project_file_contents_,
        "cpack_package_name")
    if not base_name:
        base_name = devenv.path_names.project_name_from_project_name(
            project_name)
    ...
    architecture = devenv.project.platform()[1]
...
def build_package(
    ...
    command = "cpack -G {} -C {}".format(package_generator_name,
        build_type)
    output = devenv.process.execute2(command,
...
def merge_packages(
    ...
    for source_package_directory_path_name in \
            source_package_directory_path_names:
        devenv.filesystem.copy_different_or_equal_files(
...
def check_existance_of_files_and_directories(
    ...
    try:
        root_directory_names, root_file_names = \
            devenv.filesystem.file_names_in_root_of_directory(prefix)
